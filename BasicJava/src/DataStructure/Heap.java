...
public class Heap {   
	...
	public static void heapDown(List<Integer> heap, int index) {   
		...
		//记录最大的那个儿子节点的位置   
		int child = -1;   
		if (2 * index > n) {   				//2*index>n说明该节点没有左右儿子节点了，那么就返回
			return;   
		} else if (2 * index < n) {   	//如果左右儿子都存在   
			child = 2 * index;   			//定义左儿子节点
			if ((Integer) heap.get(child) < (Integer) heap.get(child + 1)) {   //如果左儿子小于右儿子的数值，取右儿子的下标 
				child++;   
			}   
		}else if (2 * index == n) {   	//如果只有一个儿子（左儿子节点）   
			child = 2 * index;   
		}   

		if ((Integer) heap.get(child) > (Integer) heap.get(index)) {   
			swap(heap, child, index);   	//交换堆中的child，和index位置的值
			heapDown(heap, child);   			//完成交换后递归调用，继续下降  
		}   
	}   
	...
	public static void heapDown2(List<Integer> heap, int index) {   
		...
		for (; 2 * index <= n; index = child) {   //如果有儿子的话，index指向变动的孩子节点 
			child = 2 * index;   //获取左儿子的位置   
			if (child == n) {   //如果只有左儿子  
				child = 2 * index;   
			} else if ((Integer) heap.get(child) < (Integer) heap.get(child + 1)) {   //如果右儿子比左儿子的数值大
				child++;   
			}   

			if ((Integer) heap.get(child) >temp) {   //如果右儿子比左儿子的数值大
				swap(heap, child, index);   //交换堆中的child，和index位置的值   
			} else {   
				break;   
			}   
	 ...
	 */
	public static void initHeapAdjust(List<Integer> heap){  
		System.out.println("=================================================");  
		System.out.print("调整前的初始堆：");  
		print(heap);  
		for (int i = heap.size() / 2; i > 0; i--) {   
			adjust(heap,i, heap.size()-1);    
		}

		System.out.print("调整后的初始堆：");  
		print(heap);  
		System.out.println("=================================================");  
	}  
	/**  
	...
	public static void main(String args[]) {   
		...
		List<Integer> array2 = new ArrayList<Integer>(Arrays.asList(null,16,7,3,20,17,8));  
		List<Integer> array = new ArrayList<Integer>(Arrays.asList(null,1, 2, 5, 10, 3, 7, 11, 15, 17, 20, 9, 15, 8, 16));  

		initHeapAdjust(array);//初始化使array成为最大堆  
